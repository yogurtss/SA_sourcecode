...
def sleep_stage_test(X_test, Y_test):
    with tf.Graph().as_default() as g:
        ...
        Y_prediction = BN_forward_4.forward_prob(X, parameters, 1, is_training = False)
        saver = tf.train.Saver()
        correct_prediction = tf.equal(tf.argmax(Y_prediction, axis = 1), tf.argmax(Y, axis = 1))
        accuracy = tf.reduce_mean(tf.cast(correct_prediction, "float"))
        with tf.Session() as sess:
            ckpt = tf.train.get_checkpoint_state(BN_backward_4.Modelpath)
...
def main():
    ...
    labels_one = BN_backward_4.one_hot_matrix(labels, 6)
    Y_test= labels_one[:,80000:].T
