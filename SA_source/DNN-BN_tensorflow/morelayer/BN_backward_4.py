...
Modelpath = 'D:\\sa\\model\\BN_4layers'
Modelname = 'BN_4layers'
logpath = 'D:\\sa\\model\\BN_4layers\\log'
...
def L_model(X_train, Y_train, X_test, Y_test,  learning_rate = 0.0001, num_epoch = 2000, mini_batch_size = 64, kb = 0.5, print_cost = True, is_train = False):
    ...
    parameters = BN_forward_4.initialize_para()
    global_step = tf.Variable(0, trainable= False)
    ZL = BN_forward_4.forward_prob(X, parameters, keep_prob, is_training)
...
def main():
    ...
    #X_train = EEG[:, 20942:].T #for 2 channel
    ...
    Y_dev = labels_one[:, 80000:90000].T
    file.close()
    parameters, results = L_model(X_train, Y_train, X_dev, Y_dev, learning_rate=0.0001,
                                  num_epoch=2001, mini_batch_size=64, kb=0.5, print_cost=True, is_train = True)
