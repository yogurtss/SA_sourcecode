...
class DNN_model():
    ...
    def random_under_sampling(self, X_train, Y_train, seq_w, seq_rem, seq_s1, seq_s2, seq_sws):
        ...
        n_s2 = 2
        n_sws = 5
        X_train_random = np.zeros((80000 + 5565 * n_rem + 2142 * n_s1 + 12862 *n_s2 + 4188 * n_sws, 64), dtype = np.float32)
        #X_rem = np.zeros((5565 * n_rem, 64))
        #X_s1 = np.zeros((2142 * n_s1, 64))
        #X_s2 = np.zeros((12862 * n_s2))
        #X_sws = np.zeros((4188 * n_sws, 64))
        X_rem = np.repeat(X_train[seq_rem, :], n_rem, axis =0)
        X_s1 =  np.repeat(X_train[seq_s1, :], n_s1, axis = 0)
        X_s2 = np.repeat(X_train[seq_s2, :], n_s2, axis = 0)
        X_sws = np.repeat(X_train[seq_sws, :], n_sws, axis = 0)
        X_train_random[0:80000, :] = X_train
        X_train_random[80000: 80000 + 5565 * n_rem, :] = X_rem
        X_train_random[80000 + 5565 * n_rem : 80000 + 5565 * n_rem + 2142 * n_s1, :] = X_s1
        X_train_random[80000 + 5565 * n_rem + 2142 * n_s1 : 80000 + 5565 * n_rem + 2142 * n_s1 + 12862 *n_s2, :] = X_s2
        X_train_random[80000 + 5565 * n_rem + 2142 * n_s1 + 12862 *n_s2 : 80000 + 5565 * n_rem + 2142 * n_s1 + 12862 *n_s2 + 4188 * n_sws, :] = X_sws

        Y_train_random = np.zeros(80000 + 5565 * n_rem + 2142 * n_s1 + 12862 *n_s2 + 4188 * n_sws, dtype = np.float32)
        Y_train_random[0:80000] = np.argmax(Y_train, axis=1)
        X_train_random[80000: 80000 + 5565 * n_rem] = 3
        X_train_random[80000 + 5565 * n_rem: 80000 + 5565 * n_rem + 2142 * n_s1] = 2
        X_train_random[80000 + 5565 * n_rem + 2142 * n_s1: 80000 + 5565 * n_rem + 2142 * n_s1 + 12862 * n_s2] = 1
        X_train_random[
        80000 + 5565 * n_rem + 2142 * n_s1 + 12862 * n_s2: 80000 + 5565 * n_rem + 2142 * n_s1 + 12862 * n_s2 + 4188 * n_sws] = 0
    ...
    def model_train_DNN(self, X_train, Y_train, X_test, Y_test):
        ...
        model.fit(X_train_random, Y_train_random, batch_size=self.batch_size, epochs=self.epochs, shuffle=True)
