...
class DNN_model():
    def __init__(self, data_dir, label_dir, model_dir, status = True, **kwargs):
        ...
        self.seed_num = kwargs.get('seed_num')

        if status == True:
            #X_train, Y_train, X_test, Y_test = self.get_train_data()

            X_train, X_test, Y_train, Y_test = self.get_train_data(seed_num=self.seed_num)
            self.model_train_DNN(X_train, X_test, Y_train, Y_test)
            #K.clear_session()
            gc.collect()
        ...
        if status == False:
            if self.model_dir == None:
                ...
                #self.num_verify = int(input('please input the start verify number'))
    ...
    def random_pick(self, EEG, labels_one, seed_num):
        ...
        X_train = EEG[num_train, :]
        X_test = EEG[num_test, :]
        Y_train = labels_one[num_train, :]
        Y_test = labels_one[num_test, :]
        return X_train, X_test, Y_train, Y_test


    def get_train_data(self, seed_num):
        ...
        file.close()
        X_train, X_test, Y_train, Y_test = self.random_pick(EEG, labels_one, seed_num)
        '''
    ...
    def model_train_DNN(self, X_train, X_test, Y_train, Y_test):
        ...
        print('now verify the general. number is %s'%(self.seed_num + 1))
        #X_train, X_test, Y_train, Y_test = self.get_train_data(seed_num = i)
