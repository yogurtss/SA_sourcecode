...
class CNN_model():
    def __init__(self, data_dir, model_dir, status = True, **kwargs):
        ...
        if status == False:
            if self.model_dir == None:
                ...
                self.num_verify = self.seed_num
                print('number of verify %s' % self.num_verify)
    ...
    def random_pick(self, data, labels_one, seed_num):
    ...
    def get_train_data(self):
        ...
        data[:,:,2,0] = psd_1
        data[:,:,3,0] = psd_2
        X_train, X_test, Y_train, Y_test = self.random_pick(data, labels_one, self.seed_num)
        #X_train = data[0 : self.num_train, :, :, :]
        #X_test = data[self.num_train : , :, :, :]
    ...
    def get_predict_data(self):
        ...
        data[:,:,2,0] = psd_1
        data[:,:,3,0] = psd_2
        _, X, _, Y = self.random_pick(data, labels_one, self.num_verify)
        print('finished')
        return X, Y
