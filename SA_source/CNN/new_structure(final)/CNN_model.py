...
class CNN_model():
    ...
    def model_build_cnn(self, input_shape):
        ...
        model = Sequential()
        model.add(Conv1D(filters = 100, kernel_size = (50), strides = 5, padding = 'valid',
                         activation = 'elu', input_shape = input_shape, kernel_initializer = 'he_normal', ))

        model.add(BatchNormalization())
        model.add(Dropout(self.dropout_cnn))
        model.add(MaxPool1D())
        model.add(Conv1D(filters = 200, kernel_size = (10), strides = 2, padding = 'valid',
                         activation= 'elu', kernel_initializer = 'he_normal'))
        model.add(BatchNormalization())
        model.add(Dropout(self.dropout_cnn))
        model.add(MaxPool1D())
        model.add(Conv1D(filters = 300, kernel_size = (5), strides = 1, padding = 'valid',
                         activation = 'elu', kernel_initializer = 'he_normal'))
        model.add(BatchNormalization())
        model.add(Dropout(self.dropout_cnn))
        model.add(MaxPool1D())
